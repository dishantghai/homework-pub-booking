...
async def run_scenario(real: bool) -> int:
    ...
    with example_sessions_dir("ex5-edinburgh-research", persist=real) as sessions_root:
        session = create_session(
            ...
            task=(
                f"Research Edinburgh pub near {p['area']} for {p['party_size']} people, "
                f"budget £{p['budget_max_gbp']}, on {p['date']} at {p['time']}. "
                f"Catering: {p['catering_tier']}, {p['duration_hours']}h. Produce HTML flyer."
            ),
        ...
        executor_system_prompt = (
            ...
            "SESSION MEMORY: tool results are persisted across subgoals. If your\n"
            "subgoal needs data from a previous subgoal, call `recall_research`\n"
            "FIRST. Do NOT guess or fabricate values that a prior tool produced.\n\n"
            "Do NOT call `handoff_to_structured` — this is a loop-only scenario.\n"
            "You MUST call `generate_flyer` before calling `complete_task`."
        )
